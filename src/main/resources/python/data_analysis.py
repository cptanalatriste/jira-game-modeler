...
def plot_external_event(data_frame, event_column, ax=None, prefix=''):
...
def sample_tester_productivity(release_data_frame):
     """ Returns all the data points regarding issues reported per time frame
     """
     return release_data_frame[tester_prod_column]
...
def plot_severity_fixes(issues_data_frame, priority):
    ...
    nonseverity_data_frame[releases_to_fix_column].fillna(-1, inplace=True)    
...
if __name__ == '__main__':
    ...
    width_height = (20, 60)
    fig, axes = plt.subplots(10, 1, figsize=width_height)
    ...
    plot_external_event(release_data_frame, dev_prod_column, axes[3])
