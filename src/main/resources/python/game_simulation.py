...
class StochasticInflationStrategy:
    ...
    def report(self, issues_found):
        ...
        severe_reported = issues_found.severe
        default_reported = issues_found.default
        nonsevere_reported = issues_found.non_severe    
       
        default_inflation = self.inflate(issues_found.default, self.for_default)
        nonsevere_inflation = self.inflate(issues_found.non_severe, self.for_nonsevere)        
...
class Tester:
    ...
    def report(self, issue_list):
        ...
        self.record(test_reports=report, inflation_reports=inflation)
        
        return report

    def record(self, test_reports=[], inflation_reports=[],fix_reports=[]):
        ...
        self.inflation_reports.append(inflation_reports)
...
class SoftwareTesting:
    ...
    def assign_priority(self):
        ...
        maximum = sum(self.priority_probabilities)
        picker = random.uniform(0, maximum)
        partial_sum = 0
        
        for priority, probability in self.priority_probabilities:
...
if __name__ == "__main__":
    ...
    test_productivity_data = data_analysis.sample_tester_productivity(release_data_frame)
    test_productivity_kernel = stats.gaussian_kde(test_productivity_data)

    dev_team = DeveloperTeam(dev_productivity_kernel)
    truthful_strategy = StochasticInflationStrategy(0, 0)
 
    tester_team = [Tester(truthful_strategy) for _ in range(NUMBER_OF_TESTERS)]
